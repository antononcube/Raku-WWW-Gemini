...
.say for %*ENV.grep({ $_.key ~~ / GEMINI .* API | PALM .* API / });
...
my $res;
try {
    $res = gemini-generation([user => $prompt, model => 'Ok.', user => 'Who are you?'], format => 'values');
}
...
$res = gemini-generation('Who are you?', format => 'values');
